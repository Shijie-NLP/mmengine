...
VisBackendsType = Union[list[Union[list, BaseDataElement]], BaseDataElement, dict, None]
...
class Visualizer(ManagerMixin):
    ...
    def __init__(
        ...
        fig_save_cfg=dict(frameon=False),  # noqa
        fig_show_cfg=dict(frameon=False),  # noqa
    ) -> None:
        super().__init__(name)
    ...
    @staticmethod
    def _initialize_fig(fig_cfg) -> tuple:
    ...
    def draw_lines(
        ...
        if not self._is_position_valid(lines):
    ...
    def draw_circles(
        ...
        if not (
            self._is_position_valid(center - np.tile(radius.reshape((-1, 1)), (1, 2)))
            and self._is_position_valid(center + np.tile(radius.reshape((-1, 1)), (1, 2)))
        ):
            warnings.warn(
    ...
    def draw_bboxes(
        ...
        if not self._is_position_valid(bboxes.reshape((-1, 2, 2))):
    ...
    def draw_polygons(
        ...
        for polygon in polygons:
            if not self._is_position_valid(polygon):
                warnings.warn(
    ...
    def draw_featmap(
            ...
            if overlaid_image.shape[:2] != featmap.shape[1:]:
                ...
                if resize_shape is None:
                    featmap = F.interpolate(
                        featmap[None], overlaid_image.shape[:2], mode="bilinear", align_corners=False
                    )[0]
        ...
        if channel_reduction is not None:
            ...
            assert row * col >= topk, (
                "The product of row and col in the `arrangement` is less than topk, "
                "please set the `arrangement` correctly"
            )
