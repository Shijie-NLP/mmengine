...
class PretrainedInit:
    ...
    def __call__(self, module):
        from mmengine.runner.checkpoint import _load_checkpoint_with_prefix, load_checkpoint, load_state_dict

        if self.prefix is None:
            print_log(f"load model from: {self.checkpoint}", logger="current")
            load_checkpoint(module, self.checkpoint, map_location=self.map_location, strict=False, logger="current")
        else:
