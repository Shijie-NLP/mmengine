...
class BaseModel(BaseModule):
    ...
    def __init__(self, data_preprocessor: Optional[Union[dict, nn.Module]] = None, init_cfg: Optional[dict] = None):
    ...
    def to(self, *args, **kwargs) -> nn.Module:
        ...
        if args and isinstance(args[0], str) and "npu" in args[0]:
            ...
            device_name = torch_npu.npu.native_device if hasattr(torch_npu.npu, "native_device") else "privateuseone"
            args = tuple([list(args)[0].replace("npu", device_name)])  # noqa
        if kwargs and "npu" in str(kwargs.get("device", "")):
            import torch_npu

            device_name = torch_npu.npu.native_device if hasattr(torch_npu.npu, "native_device") else "privateuseone"
            kwargs["device"] = kwargs["device"].replace("npu", device_name)
    ...
    @abstractmethod
    def forward(
        self, inputs: torch.Tensor, data_samples: Optional[list] = None, mode: str = "tensor"
