...
class BaseModule(nn.Module, metaclass=ABCMeta):
    ...
    def init_weights(self):
        ...
        if not self._is_init:
            if self.init_cfg:
                print_log(
                    f"initialize {module_name} with init_cfg {self.init_cfg}", logger="current", level=logging.DEBUG
                )
            ...
            for m in self.children():
                ...
                if hasattr(m, "init_weights") and not getattr(m, "is_init", False):
                    ...
                    update_init_info(
                        m, init_info=f"Initialized by user-defined `init_weights` in {m.__class__.__name__} "
                    )
