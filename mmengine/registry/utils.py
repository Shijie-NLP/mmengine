...
def traverse_registry_tree(registry: Registry, verbose: bool = True) -> list:
    ...
    def _dfs_registry(_registry):
        if isinstance(_registry, Registry):
            ...
            if verbose:
                print_log(f"Find {num_modules} modules in {scope}'s '{_registry.name}' registry ", logger="current")
...
def count_registered_modules(save_path: Optional[str] = None, verbose: bool = True) -> dict:
    ...
    scan_data = dict(scan_date=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), registries=registries_info)
    if verbose:
...
def init_default_scope(scope: str) -> None:
    ...
    if current_scope.scope_name != scope:  # type: ignore
        print_log(
            f"The current default scope '{current_scope.scope_name}' is not '{scope}', "
            "`init_default_scope` will force set the current default scope to '{scope}'. ",
            logger="current",
