...
class ConfigDict(Dict):
    ...
    def __reduce_ex__(self, proto):
        ...
        if digit_version(platform.python_version()) < digit_version("3.8"):
            return (self.__class__, ({k: v for k, v in super().items()},), None, None, None)  # noqa
        else:
            return (self.__class__, ({k: v for k, v in super().items()},), None, None, None, None)  # noqa

    def __eq__(self, other):
        if isinstance(other, ConfigDict):
            return other.to_dict() == self.to_dict()
        elif isinstance(other, dict):
            return {k: v for k, v in self.items()} == other  # noqa
        else:
            return False
